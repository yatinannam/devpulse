...
func Analyze(entries []traffic.Request) []Finding {
 ...
 if errors>0 { findings=append(findings,Finding{"ERROR","HTTP errors",fmt.Sprintf("%d of %d requests returned 4xx/5xx",errors,len(entries))}) }
 ...
 type dup struct{k key; count int; first,last time.Time}
 var ds []dup
 ...
 for _,d:=range ds { findings=append(findings,Finding{"WARN","Duplicate requests",fmt.Sprintf("%s %s called %d times in %.1fs",d.k.method,d.k.path,d.count,d.last.Sub(d.first).Seconds())}) }
 return findings
}
