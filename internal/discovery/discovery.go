...
func probe(port int,timeout time.Duration)(bool,string,string){
 ...
 resp,err:=client.Do(req);if err!=nil{return false,"",""}
...
func identify(h http.Header,body string)string{
 ...
 if strings.Contains(server,"uvicorn")||strings.Contains(x,"fastapi"){return "Python / FastAPI"}
 if strings.Contains(server,"gunicorn"){return "Python / Gunicorn"}
 if strings.Contains(server,"next")||strings.Contains(x,"__next_data__")||strings.Contains(x,"_next/static"){return "Node / Next.js"}
 if strings.Contains(server,"vite")||strings.Contains(x,"@vite")||strings.Contains(x,"vite/client"){return "Node / Vite"}
 if strings.Contains(server,"express"){return "Node / Express"}
 if strings.Contains(server,"jetty")||strings.Contains(server,"tomcat")||strings.Contains(x,"spring"){return "Java / Spring"}
 if strings.Contains(server,"caddy"){return "Caddy"}
